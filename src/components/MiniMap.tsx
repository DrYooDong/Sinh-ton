...
export const MiniMap: React.FC<MiniMapProps> = ({
  ...
  return (
    <>
      {/* Mobile Mini Chip (sm:hidden when collapsed) */}
      {!isExpanded && (
        <div className="sm:hidden pointer-events-auto select-none">
          <button
            onClick={() => setIsExpanded(true)}
            className="flex items-center gap-1.5 bg-[#09090d]/90 hover:bg-[#13131a] border border-[#00f2ff]/50 rounded-full px-2.5 py-1 shadow-lg backdrop-blur-sm active:scale-95 transition"
          >
            <div className="relative flex items-center justify-center">
              <Radio className="w-3 h-3 text-[#00f2ff] animate-pulse" />
              <div className="absolute w-1 h-1 rounded-full bg-[#00f2ff] animate-ping" />
            </div>
            <span className="text-[10px] font-black text-[#00f2ff]">RADAR</span>
            <span className="text-[9px] font-mono text-gray-300">
              {currentKm.toFixed(1)}k
            </span>
          </button>
        </div>
      )}

      {/* Full Desktop Radar or Expanded Mobile Modal */}
      <div
        id="highway-mini-map"
        className={`transition-all duration-300 pointer-events-auto select-none ${
          isExpanded
            ? 'fixed inset-x-2 sm:inset-x-4 top-14 sm:top-20 z-40 bg-[#08080c]/95 border-2 border-[#00f2ff]/60 rounded-xl p-3 sm:p-4 shadow-2xl backdrop-blur-md max-w-5xl mx-auto'
            : 'hidden sm:block bg-[#09090d]/90 border border-[#00f2ff]/40 rounded-lg p-2.5 shadow-xl backdrop-blur-sm w-72 md:w-80'
        }`}
      >
      {/* Top Header Bar */}
      <div className="flex items-center justify-between gap-1 sm:gap-2 mb-1.5 sm:mb-2">
        <div className="flex items-center gap-1.5">
          <div className="relative flex items-center justify-center">
            <Radio className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#00f2ff] animate-pulse" />
            <div className="absolute w-1.5 h-1.5 rounded-full bg-[#00f2ff] animate-ping" />
          </div>
          <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-[#00f2ff] flex items-center gap-1">
            <span className="hidden sm:inline">RADAR CAO TỐC</span>
            <span className="sm:hidden">RADAR</span>
            <span className="text-[8px] sm:text-[9px] font-mono text-gray-400 font-normal">
        ...
        <div className="flex items-center gap-1">
          {/* Zoom Toggles */}
          <div className="flex items-center bg-[#13131a] rounded p-0.5 border border-[#232330] text-[8px] sm:text-[9px] font-mono">
            {[2, 5, 15].map((z) => (
              <button
                key={z}
                onClick={() => setZoomKm(z)}
          ...
          <button
            ...
            className="p-0.5 sm:p-1 rounded bg-[#13131a] border border-[#232330] text-gray-400 hover:text-[#00f2ff] text-[9px] sm:text-[10px]"
          >
            ?
          </button>
          ...
          <button
            ...
            className="p-0.5 sm:p-1 rounded bg-[#13131a] border border-[#232330] text-gray-400 hover:text-white"
          >
      ...
      >
        ...
        {visibleHazards.map((hazard, idx) => {
          ...
          return (
            <div
              key={`hazard_${hazard.id}_${hazard.x}_${idx}`}
        ...
        {visibleStations.map((st, idx) => {
          ...
          return (
            <div
              key={`station_${st.id}_${st.x}_${idx}`}
        ...
        {visibleChests.map((chest, idx) => {
          ...
          return (
            <div
              key={`chest_${chest.id}_${chest.x}_${idx}`}
